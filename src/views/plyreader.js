import BinaryHelper from '@kitware/vtk.js/IO/Core/BinaryHelper';
...
function postProcess(buffer, elements) {
  ...
  const duplicatePointsForFaceTexture = true
  // Ensure each face has unique vertices for texture coordinates
  if (!hasVertTCoords && hasFaceTCoords && duplicatePointsForFaceTexture) {
    let idxVerts = 0;
    let idxCoord = 0;
...
export function extend(publicAPI, model, initialValues = {}) {
    ...
    macro.setGet(publicAPI, model, ['dataAccessHelper', 'duplicatePointsForFaceTexture']);
